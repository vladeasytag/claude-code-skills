#!/usr/bin/env python3
"""One-time OAuth login for a Gmail account (account-aware).

Usage: python auth.py [account]   (account = agent | owner; default agent)
...
def get_credentials(account=DEFAULT_ACCOUNT, interactive=True):
    ...
    if os.path.exists(tok):
        ...
        # can break refresh with invalid_scope.
    ...
    if not os.path.exists(CREDENTIALS):
        sys.exit(f"Missing {CREDENTIALS} — create a Desktop OAuth client and save it as credentials.json "
                 "(see credentials.example.json and the README).")
