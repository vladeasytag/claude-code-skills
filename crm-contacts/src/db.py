...
Tables:
  ...
  emails(id PK, account, internal_date, date, from_addr, to_addr, cc, subject,
         labels, has_attachment, attachments, body, fetched)  -- the email archive
  processed(msg_id PK, processed_at)
  meta(key PK, value)
"""
import sqlite3, os, csv, json
from config import CONTACTS_DB, CONTACTS_CSV, strip_quoted
...
def conn():
    ...
    c.execute("PRAGMA busy_timeout=60000")   # wait on locks (cron + rebuild may write concurrently)
...
def _email_row_to_rec(row):
    """DB row -> corpus record dict (the shape consumers expect)."""
    return {
...
def thread(c, thread_id):
    """All archived messages in one Gmail conversation, oldest-first."""
